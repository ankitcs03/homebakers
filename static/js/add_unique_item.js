document.getElementById('addUniqueItemForm').addEventListener('submit', function(event) {
    ...
    .then(data => {
        if (data.success) {
            ...
            const table = document.getElementById('uniqueItemsTable').getElementsByTagName('tbody')[0];
            ...
            barcodeImg.src = `/static/barcodes/${data.item.barcode}.png`;
            ...
            showButton.onclick = function() { showItem(data.item.id); };
            actionsCell.appendChild(showButton);
            const editButton = document.createElement('button');
            editButton.className = 'action-button';
            editButton.innerText = 'Edit';
            editButton.onclick = function() { editItem(data.item.id); };
            actionsCell.appendChild(editButton);
            const deleteButton = document.createElement('button');
            deleteButton.className = 'delete-button';
            deleteButton.innerText = 'Delete';
            deleteButton.onclick = function() { deleteItem(data.item.id); };
...
function showItem(itemId) {
    ...
    .then(data => {
        if (data.success) {
            ...
            document.getElementById('detail_description').innerText = data.item.description;
...
function editItem(itemId) {
    ...
    .then(data => {
        if (data.success) {
            ...
            document.getElementById('modal_description').value = data.item.description;
            ...
            document.getElementById('editItemForm').onsubmit = function(event) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        row.cells[3].querySelector('img').src = `/static/barcodes/${data.item.barcode}.png`;
